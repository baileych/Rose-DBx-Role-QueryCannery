...
BEGIN {
  ...
  if ( eval { require Rose::DBx::CannedQuery::Glycosylated } ) {
    ...
    Test::More->import( skip_all => 'No Rose::DBx::CannedQuery::Glycosylated' );
  }
}
...
$qry = $cannery->build_query( 'SELECT * FROM test WHERE color = ?',
                            { verbose => 3, logger => My::Test::Logger->new,
			      name => 'logger_test' } );
